/*
 * a simple kernel module: interrupt
 *
...
static irqreturn_t irq_thread_fn(int irq, void *dev_data)
{
	struct key *k = dev_data;
	printk(KERN_INFO "In irq thread, key [%s], irq %d\n", k->desc, k->irq);
	return IRQ_HANDLED;
}
...
struct platdata *key_get_devtree_pdata(struct device *dev)
{
	...
	for_each_child_of_node(node, pp) {
		...
		printk(KERN_INFO "find a new key: irq %d, desc [%s]\n", k->irq, k->desc);
		k++;
	}
	return pdata;
}
...
static int gpio_keys_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->nr_key; i++, k++) {
		ret = request_threaded_irq(k->irq, irq_handler, irq_thread_fn,
					 IRQF_SHARED, k->desc, k);
		printk(KERN_INFO "request irq %d for key [%s] %s\n", k->irq, k->desc, ret ? "fail" : "ok");
	}
